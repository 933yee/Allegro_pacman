...
extern ALLEGRO_SAMPLE* PACMAN_POWERBEANSOUND;
...
static bool pacman_movable(Pacman* pacman, Map* M, Directions targetDirec) {
	...
	Pair_IntInt TEM = { (pacman->objData.Coord.x),(pacman->objData.Coord.y) };
	...
	if (is_wall_block(M, TEM.x, TEM.y) || is_room_block(M, TEM.x, TEM.y))
		return false;
	...
	if (game_over) {
		...
		*/
		if (al_get_timer_started(pman->death_anim_counter)) {
			...
			int a = al_get_timer_count((pman->death_anim_counter));
			if (a >= 72) {
				al_stop_timer(pman->death_anim_counter);
				return;
			}
			if (a/6*6 == a) {
				al_draw_scaled_bitmap(pman->die_sprite, 16 * (a/6), 0,
					16, 16,
		...
		int frame = (pman->objData.moveCD / 32);
		frame = frame - frame / 2 * 2;
		al_draw_scaled_bitmap(pman->move_sprite, 16 * frame + pmanfacing, 0,
			16, 16,
			drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
			draw_region, draw_region, 0
		);
