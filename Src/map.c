...
Map* create_map(const char* filepath) {
	...
	if (filepath == NULL) {
		...
		if (level == 0) {
			...
			pFile = fopen("Assets/map_nthu2.txt", "r");
		}
		else if (level == 2) {
			pFile = fopen("Assets/map_nthu3.txt", "r");
		}
...
static void draw_bean(Map* M, const int row, const int col) {
	...
	al_draw_filled_circle(map_offset_x + col * block_width + block_width / 2.0, map_offset_y + row * block_height + block_height / 2.0, block_width / 6.0, al_map_rgb(r, g, b));
}
...
static void draw_power_bean(Map* M, const int row, const int col) {
	...
	g = 255;
	b = 255;
	al_draw_filled_circle(map_offset_x + col * block_width + block_width / 2.0, map_offset_y + row * block_height + block_height / 2.0, block_width / 3.0, al_map_rgb(r, g, b));
}
