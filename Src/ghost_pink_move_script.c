...
static void ghost_pink_move_script_FREEDOM(Ghost* ghost, Map* M) {
	...
	int move = rand() % 4;
	if (ghost->objData.preMove + proba[move] == 5) {
		move = (move + 1) % cnt;
	}
...
void ghost_pink_move_script(Ghost* ghost, Map* M, Pacman* pacman) {
	...
	if (!movetime(ghost->speed))
		...
		case BLOCKED:
			...
			break;
		case FREEDOM:
			ghost_NextMove(ghost, shortest_path_direc(M, ghost->objData.Coord.x, ghost->objData.Coord.y, pacman->objData.Coord.x, pacman->objData.Coord.y));
			break;
		case GO_OUT:
