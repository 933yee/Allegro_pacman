...
void ghost_draw(Ghost* ghost) {
	...
	*/
	...
	if (ghost->status == FLEE) {
		if (al_get_timer_count(power_up_timer)==7 || al_get_timer_count(power_up_timer) == 9 ) {
			int ghost_facing = 32;
			int frame = (ghost->objData.moveCD / 16);
			frame = frame - frame / 2 * 2;
			al_draw_scaled_bitmap(ghost->flee_sprite, 16 * frame + ghost_facing, 0,
				16, 16,
		...
		al_draw_scaled_bitmap(ghost->dead_sprite, ghost_facing, 0,
			16, 16,
			drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
			draw_region, draw_region, 0
		);
		...
		int frame = (ghost->objData.moveCD/16);
		frame = frame - frame / 2 * 2;
		al_draw_scaled_bitmap(ghost->move_sprite, 16*frame+ ghost_facing, 0,
			16, 16,
			drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
			draw_region, draw_region, 0
		);
...
bool ghost_movable(Ghost* ghost, Map* M, Directions targetDirec, bool room) {
	...
	Pair_IntInt TEM2 = { (ghost->objData.Coord.x),(ghost->objData.Coord.y) };
	...
	if (is_wall_block(M, TEM2.x, TEM2.y) || (room && is_room_block(M, TEM2.x, TEM2.y)))
		return false;
...
void ghost_move_script_FLEE(Ghost* ghost, Map* M, const Pacman * const pacman) {
	...
	int move = rand() % 4 ;
	if (ghost->objData.preMove + proba[move]==5 || proba[move]== shortestDirection){
		move = (move + 1) % cnt;
	}
